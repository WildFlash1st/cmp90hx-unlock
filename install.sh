...
Without --profile, stock nvidia-smi memory.total selects the profile:
  ...
  ~10240 MiB → 10gb / 40GB unlock or cmp90 (depends on PCI ID)
EOF
            exit 0
            ;;
        *)
...
step "Step 2/6: Detecting CMP GPU"
PCI_LINE="$(lspci -nn 2>/dev/null | grep -iE '10de:20b0|10de:20c2|10de:2082|10de:220d' | head -1 || true)"
[[ -n "${PCI_LINE}" ]] || die "No CMP GPU found (10de:20b0 / 10de:20c2 / 10de:2082 / 10de:220d)"
...
IS_CMP90=0
if [[ "${DEVID}" == "20b0" || "${DEVID}" == "220d" ]]; then
    IS_CMP90=1
    info "CMP 90 detected (GA102, PCI ID 0x${DEVID^^}) — compute unlock mode"
elif [[ "${DEVID}" != "20c2" && "${DEVID}" != "2082" ]]; then
    warn "In-driver unlock path is gated on PCI ID 0x20C2 / 0x2082 / 0x20B0 / 0x220D."
...
if [[ -z "${detected}" ]] && command -v nvidia-smi &>/dev/null; then
    raw="$(nvidia-smi --query-gpu=driver_version --format=csv,noheader 2>/dev/null | head -1 | tr -d '[:space:]' || true)"
...
echo -e "  3. Verify unlock logs: ${CYAN}sudo dmesg | grep CMP90_DEBUG${NC}"
